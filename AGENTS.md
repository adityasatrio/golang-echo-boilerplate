...
- `internal/applications/<domain>`: MVC modules: `controller`, `service`, `repository`, `dto`; DI via `<domain>_injector.go` and `wire_gen.go`.
...
- Mocks/Wire: `make mocks`, `make wire` (choose module interactively).
...
- Go 1.19; format with `go fmt`; idiomatic Go. Tabs for indentation.
- Packages lower-case; files `snake_case.go`. Exported: PascalCase; unexported: camelCase.
- Domain naming: `<domain>_controller.go`, `<domain>_routes.go`, `<domain>_service*.go`, `<domain>_repository*.go`, `<domain>_injector.go`.
- Do not edit generated files (`wire_gen.go`, `ent/*` except `ent/schema`).
