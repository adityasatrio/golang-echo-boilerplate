...
// The AtlasSchemaRevisionFunc type is an adapter to allow the use of ordinary
// function as AtlasSchemaRevision mutator.
type AtlasSchemaRevisionFunc func(context.Context, *ent.AtlasSchemaRevisionMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AtlasSchemaRevisionFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.AtlasSchemaRevisionMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AtlasSchemaRevisionMutation", m)
	}
	return f(ctx, mv)
}
