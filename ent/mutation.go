...
const (
	...
	TypePet             = "Pet"
	TypePost            = "Post"
	TypeSystemParameter = "System_parameter"
	TypeUser            = "User"
)
...
// executed in a transaction (ent.Tx), a transaction client is returned.
...
func (m *PostMutation) SetStatus(i int) {
	m.status = &i
	m.addstatus = nil
}

// Status returns the value of the "status" field in the mutation.
func (m *PostMutation) Status() (r int, exists bool) {
	v := m.status
	if v == nil {
		return
	}
...
// If the Post object wasn't provided to the builder, the object is fetched from the database.
// An error is returned if the mutation operation is not UpdateOne, or the database query fails.
func (m *PostMutation) OldStatus(ctx context.Context) (v int, err error) {
	if !m.op.Is(OpUpdateOne) {
...
func (m *PostMutation) AddStatus(i int) {
	if m.addstatus != nil {
		*m.addstatus += i
	} else {
		m.addstatus = &i
...
func (m *PostMutation) AddedStatus() (r int, exists bool) {
	v := m.addstatus
	if v == nil {
		return
	}
...
func (m *SystemParameterMutation) WhereP(ps ...func(*sql.Selector)) {
	p := make([]predicate.System_parameter, len(ps))
	for i := range ps {
		p[i] = ps[i]
	}
	m.Where(p...)
